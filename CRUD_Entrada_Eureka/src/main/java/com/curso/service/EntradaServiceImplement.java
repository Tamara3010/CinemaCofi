...
public class EntradaServiceImplement implements EntradaService {
	...
	public List<EntradaNombre> peliculasEntradasDisponibles() {
		...
		for (Pelicula sp : peliculasDisponibles) {
			entradas.add(new EntradaNombre(
					template.getForObject(url_peliculas.concat("/peliculas/" + sp.getIdpelicula()), String.class),
					sp.getNumbutacas(), sp.getHora()));
		} // for
	...
	public void comprarEntrada(String nombre_pelicula, String hora_pelicula, int num_entradas) {
		...
		int id_sala_pelicula = template.getForObject(
				url_sala_pelicula + "salapelicula/" + id_pelicula + "/" + hora_pelicula + "/" + num_entradas,
				Integer.class);
